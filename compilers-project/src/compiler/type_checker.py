...
def typecheck(node: ast.Expression, typetab: SymTab) -> Type:
  def get_type() -> Type:
    ...
    match node:
      ...
      case ast.Condition():
        t1 = typecheck(node.con, typetab)
        if t1 is not Bool:
          raise Exception(f"{node.location}, expected {Bool} got {t1}")
        
        t2 = typecheck(node.then, typetab)
        if node.el is None:
          return t2
        t3 = typecheck(node.el, typetab)
        if t2 != t3:
          raise Exception(f"{node.location}, expected matching types for both branches of if, got {t1} and {t2}")
        return t2
      
      case ast.Loop():
        t1 = typecheck(node.condition, typetab)
        if t1 is not Bool:
          raise Exception(f"{node.location}, expected {Bool} got {t1}")
        t2 = typecheck(node.do, typetab)
        return t2
